    ...
    (cond
      ...
      [(empty? chars)
       (if (member state (dfa-accept dfa-to-evaluate))
           (reverse (cons state tokens)) ;cons is for returning the first value of the pair
            'invalid)]
      [else
       (let-values
       ; Call the transition function and get the new state and wheter or not a token was fo
